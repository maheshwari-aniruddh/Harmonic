...
// Listen for note events from AR iframe and forward to jam session + trigger local/remote handling
window.addEventListener('message', (event) => {
    const data = event.data;
    if (!data || (data.type !== 'notePlayed' && data.type !== 'NOTE_PLAYED')) return;
    const note = (data.note || '').toString().trim();
    const velocity = typeof data.velocity === 'number' ? data.velocity : 0.8;
    if (!note) return;

    // Forward to WebSocket if in a jam room
    if (jamSessionManager && jamSessionManager.connected) {
        jamSessionManager.sendNote(note, velocity);
    }

    // Local visual: key or drum press (backend already plays sound in iframe)
    ...
    animate() {
        ...
        this.notes.forEach((note) => {
            ...
            if (!note.hit) {
                ...
                this.ctx.shadowColor = note.isRemote ? 'rgba(0, 255, 136, 0.8)' : 'rgba(102, 126, 234, 0.8)';
            }
        ...
        if (mp3Btn && mp3Input) {
            ...
            mp3Input.addEventListener('change', function () {
                ...
                fetch(api + '/api/upload-mp3', { method: 'POST', body: fd })
                    ...
                    .catch(err => {
                        if (mp3Status) mp3Status.textContent = 'Upload failed';
                        console.error(err);
                    });
                this.value = '';
            });
...
class DrumsNoteManager {
    ...
    /** Add a falling note from a remote jam partner (distinct color). */
    updatePartition() {
    ...
    animate() {
        ...
        this.notes.forEach((note) => {
            ...
            const noteHeight = 80;
            let gradient = this.ctx.createLinearGradient(note.x, note.y, note.x, note.y + noteHeight);
            if (note.drum === 'bass1') {
...
let jamRoomId = null; // Set when host creates or client joins
...
function initJamSetup() {
    ...
    if (!jamSessionManager) jamSessionManager = new JamSessionManager();
...
function initJamPiano() {
    ...
    if (badge && jamSessionManager) badge.textContent = 'Room: ' + (jamSessionManager.roomId || '');
    document.querySelectorAll('#jam-piano-keys .piano-key').forEach(key => {
        key.addEventListener('click', () => {
            const note = key.dataset.note;
            if (jamSessionManager && jamSessionManager.connected && note) {
...
function exitJamSession() {
    if (jamSessionManager) jamSessionManager.disconnect();
    jamRoomId = null;
    const connectedDiv = document.getElementById('jam-connected');
...
(function () {
    function init() {
        ...
        // Show landing or instruments page
        showPage(document.getElementById('instruments-page') ? 'instruments-page' : 'landing');
