    ...
    while run and cap.isOpened():
        ...
        if mode == "Air Drums (4-Zone)":
            ...
            for pad in entered_pads:
                ...
                # Broadcast drum to parent for jam session
                safe_name = (drum_name or "").replace('"', '\\"')
                st.components.v1.html(
                    f'<script>if(window.parent){{window.parent.postMessage({{type:"notePlayed",note:"{safe_name}"}},"*");}}</script>',
                    height=0
                )
